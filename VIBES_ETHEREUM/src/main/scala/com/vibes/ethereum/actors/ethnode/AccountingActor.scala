...
class AccountingActor(clientId: String, evmQueue: SourceQueueWithComplete[EventJson], reducer: ActorRef) extends Actor{
  ...
  def updateStats(stats: Stats, startTimestamp: Long, block: Block): Stats = {
    ...
    try {stats.avgDifficulty_= (stats.calcAvg(stats.blockNum, stats.avgDifficulty, stats.difficulty))}
    catch {
      case e => {
        println("exception occoured")
      }
    }
  ...
  override def receive: Receive = {
    ...
    case BlockVerified(startTimestamp: Long, block: Block) => {
      stats = updateStats(stats, startTimestamp, block)
      stats.propTime_= (startTimestamp - block.timestamp)
    ...
    case EvmStarted => {evmState = EvmState.INITIALIZING; evmQueue offer EventJson(clientId, "evmState", "Initializing")}
    case EvmInitialized => {evmState = EvmState.IDLE; evmQueue offer EventJson(clientId, "evmState", "Idle")}
    ...
    case PoolerStarted => {txPoolState = TxPoolState.INITIALIZING; evmQueue offer EventJson(clientId, "poolState", "Initializing")}
    case PoolerInitialized => {txPoolState = TxPoolState.IDLE; evmQueue offer EventJson(clientId, "poolState", "Accepting Transactions")}
    ...
    case Discover => {nodeState = NodeState.DISCOVER; evmQueue offer EventJson(clientId, "nodeState", "Neighbour Discovery")}
